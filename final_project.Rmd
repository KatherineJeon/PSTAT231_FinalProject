...
```{r}
ggplot(data_count, aes(x = month, y = n, group = revenue)) + geom_line(aes(color = revenue)) + geom_point(aes(color = revenue)) + geom_text(aes(label = n), vjust = - 0.5) + theme_minimal()
```
...
best_parameter <- select_best(rf_tune, metric = "roc_auc")
rf_final <- finalize_workflow(rf_tune, best_parameter)
...
augment(rf_final_fit, new_data = data_test) %>%
  rsq(truth = revenue, estimate = .pred)
```
...
best_parameter <- select_best(bt_wf, metric = "roc_auc")
bt_final <- finalize_workflow(bt_tune, best_parameter)
...
rf_final <- finalize_workflow(rf_tune, best_parameter)
rf_final_fit <- fit(pokemon_rf_final, data = data_train)
```
